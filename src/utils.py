...
def save_plots(model, criterion, metrics_df, pretrained, num_classes, bayes_last):
    ...
    # save to csv first
    try:
        csv_save_path = f"./results/df_model_{model_name}_pretrained_{pretrained}_loss_{loss_name}_numClass_{num_classes}_Bay_{bayes_last}.csv"
    ...
    for m_name in metrics_df.index:
        ...
        save_path = f"./results/plot_model_{model_name}_{pretrained}_loss_{loss_name}_numClass_{num_classes}_Bay_{bayes_last}_metric_{y_label}.jpg"
        plt.legend()
        plt.xlabel('Epochs')
